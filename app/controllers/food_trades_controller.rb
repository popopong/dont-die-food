class FoodTradesController < ApplicationController
  ...
  def user_food_trades
    @user = current_user
    @food_trades = @user.food_trades.includes([:user_owned_ingredient])
  end

  def index
    @food_trades = FoodTrade.includes([:user_owned_ingredient]).where(status: "Available")

    @food_trades_geocoded = FoodTrade.includes([:user_owned_ingredient]).geocoded
